...
class TestBasicWorkflow:
    ...
    def test_server_initialization_workflow(
        self, mock_project_service, mock_task_service, mock_api_client
    ):
    ...
    def test_config_and_client_workflow(self):
        ...
        from ticktick_mcp.client import ProjectService, TaskService, TickTickAPIClient
        from ticktick_mcp.config import TickTickConfig

        # Create config
        config = TickTickConfig(
            client_id="test_id", client_secret="test_secret", access_token="test_token"
        )
        ...
        with patch(
            "ticktick_mcp.client.ConfigManager", return_value=mock_config_manager
        ):
