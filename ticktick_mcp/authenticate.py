...
def test_existing_credentials(config_manager: ConfigManager) -> TestResult:
    ...
    try:
        ...
        response = requests.get(
            f"{config.base_url}/project", headers=headers, timeout=10
        )
...
def main() -> int:
    ...
    services_table.add_row(
        "🌍",
        "[bold blue]TickTick[/bold blue] (International)",
        "[dim]https://developer.ticktick.com[/dim]",
    )
    services_table.add_row(
        "🇨🇳",
        "[bold red]Dida365[/bold red] (滴答清单 - China)",
        "[dim]https://developer.dida365.com[/dim]",
    )
    ...
    content = Text()
    content.append(
        "This utility will help you authenticate with TickTick or Dida365.\n\n"
    )
    content.append("Supported services:\n", style="bold")

    # Display the banner
    console.print(Panel.fit(title_text, style="bright_blue", padding=(1, 2)))
    ...
    choice_table.add_row(
        "[bold blue]1.[/bold blue]",
        "🌍 [bold blue]TickTick[/bold blue] (International)",
    )
    choice_table.add_row(
        "[bold red]2.[/bold red]", "🇨🇳 [bold red]Dida365[/bold red] (滴答清单 - China)"
    )
    ...
    try:
        ...
        if (
            ...
            and existing_config.client_secret
            and existing_config.use_dida365 == use_dida365
        ):
    ...
    if has_matching_credentials:
        ...
        with console.status(
            f"[bold blue]Testing {service_name} API connection...[/bold blue]"
        ):
            test_result = test_existing_credentials(config_manager)

        if test_result.success:
            console.print(
                "[bold green]✅ Credentials verified successfully![/bold green]"
            )
            console.print(
                "[dim]You can now run the server with:[/dim] [bold]ticktick-mcp run[/bold]"
            )
            return 0
        else:
            console.print(
                f"[bold red]❌ Credential test failed:[/bold red] {test_result.error}"
            )
        ...
        if existing_config and existing_config.client_id:
            ...
            console.print(
                "[yellow]Setting up new credentials for your selected service...[/yellow]"
            )
        else:
        ...
        if use_dida365:
            console.print(
                "[bold green]✅ Dida365 endpoints will be configured automatically[/bold green]"
            )
    ...
    auth = TickTickAuth(
        client_id=client_id, client_secret=client_secret, config_manager=config_manager
    )
    ...
    console.print(
        "[dim]A browser window will open for you to authorize the application.[/dim]"
    )
    console.print(
        "[dim]After authorization, you will be redirected back to this application.[/dim]\n"
    )
    ...
    if "successful" in result.lower():
        ...
        success_steps.add_row(
            "[bold]1.[/bold]", "Make sure you have configured Claude for Desktop"
        )
        success_steps.add_row("[bold]2.[/bold]", "Restart Claude for Desktop")
        success_steps.add_row(
            "[bold]3.[/bold]",
            "You should now see the task management tools available in Claude",
        )
