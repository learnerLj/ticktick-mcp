...
from .client import TaskService, ProjectService
from .exceptions import ValidationError, TickTickMCPError
...
class BaseMCPTool(ABC):
    ...
    def _validate_priority(self, priority: int) -> Priority:
        ...
        if priority not in [0, 1, 3, 5]:
            raise ValidationError("Priority must be 0 (None), 1 (Low), 3 (Medium), or 5 (High)")
    ...
    def _validate_view_mode(self, view_mode: str) -> ViewMode:
        ...
        try:
            ...
            raise ValidationError("View mode must be one of: list, kanban, timeline")
    ...
    def _validate_date_format(self, date_str: str, field_name: str) -> None:
        ...
        try:
            ...
            raise ValidationError(f"Invalid {field_name} format. Use ISO format: YYYY-MM-DDThh:mm:ss+0000")
...
class CreateProjectTool(BaseMCPTool):
    ...
    async def execute(self, name: str, color: str = "#F18181", view_mode: str = "list") -> str:
...
class GetTaskByIdTool(BaseMCPTool):
    ...
    def __init__(self, task_service: TaskService):
        super().__init__("get_task_by_id", "Get a task by its ID without requiring project ID")
        self.task_service = task_service
